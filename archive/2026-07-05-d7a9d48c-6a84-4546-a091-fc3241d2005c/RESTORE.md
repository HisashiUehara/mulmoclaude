# Restore "ほしい物リスト（Seo）" (collection `wishlist-seo`)

This folder is an automatic backup made when the collection was deleted.
Follow these steps to restore it.

1. Recreate the skill files in `data/skills/wishlist-seo/` using the **Write
   ...
   The skill-bridge hook mirrors `data/skills/wishlist-seo/` into
   `.claude/skills/wishlist-seo/`, and that mirror is what actually registers
...
2. Copy the item data: `cp` every file under `records/` into
   `data/wishlist-seo/items/`. The records are part of the collection and
...
3. Confirm the collection reappears at `/collections/wishlist-seo`.

- slug: `wishlist-seo`
- title: ほしい物リスト（Seo）
- dataPath: `data/wishlist-seo/items`
